...
defaultAgentSystemPrompt = T.unlines
  ...
  , ""
  , "2. **Context Mass Economy & Ephemeral Subagents**:"
  , "   - Keep dialogue context lean and high-signal (O(1) context mass principle)."
  , "   - When debugging large compiler error cascades, test logs, crash dumps, or memory sanitizer (ASan/TSan) traces, delegate the deep log inspection to an ephemeral diagnostic subagent (spawn_diagnostic_subagent) rather than pulling massive dumps directly into the main conversation."
  , "   - Subagents run in an isolated execution loop with a strict turn budget and return structured diagnostic conclusions."
  , ""
